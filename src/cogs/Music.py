...
class Music(commands.Cog):
    ...
    async def play(self, ctx, *args):
        ...
        if voice_state is None:
            await ctx.send(
                embed=create_embed(
                    "Error",
                    "You are not currently in a voice channel. Please join a voice channel to play music",
                )
            )
            return
        ...
        if len(args) == 0:
            await ctx.send(
                embed=create_embed(
                    "No Url Passed.",
                    "No url was passed to the command. Please try again.",
                )
            )
            return
        ...
        try:  # Check bot's current voice channel
            if (
                ...
                await ctx.send(
                    embed=create_embed(
                        "Already Playing",
                        f"The bot is already playing on channel `{guild_data['channel'].name}`",
                    )
                )
                return
        ...
        message = await ctx.send(
            embed=create_embed(
                "Setting Song!",
                f"Song: `{url}` is now being extracted!. The music will automatically start playing as soon as the extraction finishes.",
            )
        )
        ...
        if video_data["audio_duration"] > 3600:  # Bigger than 1 hour
            await message.edit(
                embed=create_embed(
                    "Error",
                    f"Song: `{video_data['audio_title']}` can not be played as its duration is too long. \n\nMax Duration is : `1 hour`.",
                )
            )
            return
        ...
        if len(guild_data["queue"]) > 0 or guild_data["currently_playing"] is not None:
            await message.edit(
                embed=create_embed(
                    "Added To Queue",
                    f"Song: `{video_data['audio_title']}` was added to queue.",
                )
            )
    ...
    @commands.command(
        name="Stop",
        description="Stops the current music. Disconnects the bot",
        aliases=["disconnect"],
    )
    ...
    async def pause(self, ctx):
        ...
        try:
            ...
            await ctx.send(
                embed=create_embed(
                    "Error",
                    f"Bot is not currently in a voice channel. Run the `play` command to play music.",
                )
            )
    ...
    async def loop(self, ctx):
        try:
            ...
            await ctx.send(
                embed=create_embed(
                    "Set Loop",
                    f'Loop is set to `{music_guilds[ctx.guild.id]["looped"]}`',
                )
            )
        except KeyError:
